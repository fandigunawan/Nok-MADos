...
* Reentrant:   Yes
* $Id: huffman.c 2612 2007-09-07 11:34:57Z geggo $
*
...
static void
_Huffman_InitBitstream ( huff_bitstream_t * stream, unsigned char *buf, unsigned int bytes )
{
    stream->BytePtr = buf;
    stream->BitPos = 0;
...
static unsigned int
_Huffman_ReadBits ( huff_bitstream_t * stream, unsigned int bits )
{
...
static void
_Huffman_WriteBits ( huff_bitstream_t * stream, unsigned int x, unsigned int bits )
{
...
static void
_Huffman_Hist ( unsigned char *in, huff_sym_t * sym, unsigned int size )
{
    int k, swaps;
    huff_sym_t tmp;
...
static void
_Huffman_MakeTree ( huff_sym_t * sym, huff_bitstream_t * stream, unsigned int code, unsigned int bits, unsigned int first, unsigned int last )
{
...
static void
_Huffman_RecoverTree ( huff_sym_t * sym, huff_bitstream_t * stream, unsigned int code, unsigned int bits, unsigned int *symnum )
{
    unsigned int symbol;
...
int
Huffman_Compress ( unsigned char *in, unsigned char *out, unsigned int insize )
{
...
void
Huffman_Uncompress ( unsigned char *in, unsigned char *out, unsigned int insize, unsigned int outsize )
{
    huff_sym_t sym[256], tmp;
    huff_bitstream_t stream;
...
main ( void )
{
    ...
    char toggle;
    char *inputfile, *outputfile, *filename_in, *filename_out;
    unsigned int infile_size = 0, outfile_size;
    FILE *input, *output;

    inputfile = ( char * ) malloc ( 65536 );
    outputfile = ( char * ) malloc ( 65536 );
    filename_in = ( char * ) malloc ( 256 );
    filename_out = ( char * ) malloc ( 256 );

    printf ( "type y, if you want to compress a file, type another char if you want to decompress a file: " );
    scanf ( "%c", &toggle );

    printf ( "\nplease type input filename: " );
    scanf ( "%s", filename_in );

    printf ( "\nplease type output filename: " );
    scanf ( "%s", filename_out );

    input = fopen ( filename_in, "rb" );
    output = fopen ( filename_out, "wb" );
    ...
    if ( !input || !output )
    {
        ...
        //files should be closed, mem should be free'd - but it's a user fault and he should suffer for that :D
        return 1;
    }

    if ( toggle != 'y' )
    {
        printf ( "\nplease enter lenght of uncompressed file: " );
        scanf ( "%d", &outfile_size );
    }

    while ( fread ( &inputfile[infile_size], 1, 1, input ) && infile_size < 65536 )
        infile_size++;

    if ( infile_size >= 65152 )
    {
        printf ( "\nfile exceeds allocated memory, change the source. aborting ..." );
        //files should be closed, mem should be free'd - my fault, but the user should suffer too *g*
        return 1;
    }

    printf ( "\nlenght of input file (remember it for decompressing): %d\n\n", infile_size );
    //read the file

    if ( toggle == 'y' )
    {
        outfile_size = Huffman_Compress ( ( unsigned char * ) inputfile, ( unsigned char * ) outputfile, infile_size );
        printf ( "lenght of compressed file: %d\n\ncompression: %f", outfile_size, ( ( float ) 1 - ( ( float ) outfile_size / infile_size ) ) * 100 );
    }
    else
        Huffman_Uncompress ( ( unsigned char * ) inputfile, ( unsigned char * ) outputfile, infile_size, outfile_size );

    fwrite ( outputfile, 1, outfile_size, output );
