...
inline float32
packFloat32 ( flag zSign, int16 zExp, bits32 zSig )
{
...
static void
normalizeFloat64Subnormal ( bits32 aSig0, bits32 aSig1, int16 * zExpPtr, bits32 * zSig0Ptr, bits32 * zSig1Ptr )
{
    int8 shiftCount;
...
static float64
roundAndPackFloat64 ( flag zSign, int16 zExp, bits32 zSig0, bits32 zSig1, bits32 zSig2 )
{
    int8 roundingMode;
    ...
    {
        if ( ( 0x7FD < zExp ) || ( ( zExp == 0x7FD ) && eq64 ( 0x001FFFFF, 0xFFFFFFFF, zSig0, zSig1 ) && increment ) )
        {
            float_raise ( float_flag_overflow | float_flag_inexact );
        ...
        if ( zExp < 0 )
        {
            isTiny = ( float_detect_tininess == float_tininess_before_rounding ) || ( zExp < -1 ) || !increment || lt64 ( zSig0, zSig1, 0x001FFFFF, 0xFFFFFFFF );
...
float32
float32_add ( float32 a, float32 b )
{
    flag aSign, bSign;
...
float32
float32_mul ( float32 a, float32 b )
{
...
float32
float32_div ( float32 a, float32 b )
{
    flag aSign, bSign, zSign;
    int16 aExp, bExp, zExp;
...
float32
float32_rem ( float32 a, float32 b )
{
    flag aSign, bSign, zSign;
...
char
float32_eq_signaling ( float32 a, float32 b )
{
...
char
float32_le_quiet ( float32 a, float32 b )
{
    flag aSign, bSign;
    int16 aExp, bExp;
...
char
float32_lt_quiet ( float32 a, float32 b )
{
    flag aSign, bSign;
...
float64_round_to_int ( float64 a )
{
    ...
    if ( 0x413 <= aExp )
    {
        if ( 0x433 <= aExp )
        {
            if ( ( aExp == 0x7FF ) && ( extractFloat64Frac0 ( a ) | extractFloat64Frac1 ( a ) ) )
            {
    ...
    else
    {
        if ( aExp <= 0x3FE )
        {
            ...
            {
                case float_round_nearest_even:
                    if ( ( aExp == 0x3FE ) && ( extractFloat64Frac0 ( a ) | extractFloat64Frac1 ( a ) ) )
                    {
                        return packFloat64 ( aSign, 0x3FF, 0, 0 );
                    }
                    break;
                case float_round_down:
                    return aSign ? packFloat64 ( 1, 0x3FF, 0, 0 ) : packFloat64 ( 0, 0, 0, 0 );
                case float_round_up:
                    return aSign ? packFloat64 ( 1, 0, 0, 0 ) : packFloat64 ( 0, 0x3FF, 0, 0 );
            }
            return packFloat64 ( aSign, 0, 0, 0 );
        }
        lastBitMask = 1;
...
float64
float64_sub ( float64 a, float64 b )
{
    flag aSign, bSign;
...
float64
float64_rem ( float64 a, float64 b )
{
    flag aSign, bSign, zSign;
    int16 aExp, bExp, expDiff;
    ...
    if ( expDiff < -1 )
        return a;
    shortShift64Left ( aSig0 | 0x00100000, aSig1, 11 - ( expDiff < 0 ), &aSig0, &aSig1 );
    shortShift64Left ( bSig0 | 0x00100000, bSig1, 11, &bSig0, &bSig1 );
    ...
    add64 ( aSig0, aSig1, alternateASig0, alternateASig1, &sigMean0, &sigMean1 );
    ...
    return normalizeRoundAndPackFloat64 ( aSign ^ zSign, bExp - 4, aSig0, aSig1 );
...
float64_eq ( float64 a, float64 b )
{
    ...
    if ( ( ( extractFloat64Exp ( a ) == 0x7FF )
           && ( extractFloat64Frac0 ( a ) | extractFloat64Frac1 ( a ) ) ) || ( ( extractFloat64Exp ( b ) == 0x7FF ) && ( extractFloat64Frac0 ( b ) | extractFloat64Frac1 ( b ) ) ) )
    {
...
char
float64_eq_signaling ( float64 a, float64 b )
{
...
char
float64_le_quiet ( float64 a, float64 b )
{
    flag aSign, bSign;
...
char
float64_lt_quiet ( float64 a, float64 b )
{
    flag aSign, bSign;
